...
#ifndef BUTTON_PIN_3
#error BUTTON_PIN_3 is not specified
#endif
...
const int password_period = 7;
ETSTimer password_timer;
...
void gpio_init() {
    ...
    gpio_enable(BUTTON_PIN_3, GPIO_INPUT);
}

static void password_callback(void *arg) {
...
void button_3_callback(button_event_t event, void* context) {
    switch (event) {
        case button_event_single_press:
          ...
          break;
        case button_event_double_press:
          printf("Button_pin_3 double press\n");
          break;
        case button_event_long_press:
          printf("Unlocking\n");
          relay_write(true);
...
void user_init(void) {
    ...
    if (button_create(BUTTON_PIN_3, button_config, button_3_callback, NULL)) {
       printf("Failed to initialize button\n");
        }
